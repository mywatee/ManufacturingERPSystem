...
public class PermissionService : IPermissionService
{
    ...
    public async Task<List<RolePermissionDto>> GetRolePermissionsAsync(string roleName)
    {
        ...
        if (role == null) return DefaultPermissions();
        ...
        foreach (var (key, displayName) in SystemModules.All)
        {
            ...
            {
                result.Add(new RolePermissionDto(key, displayName, CanView: true, CanAdd: false, CanEdit: false, CanDelete: false));
            }
        }
    ...
    public async Task ResetRolePermissionsAsync(string roleName)
    {
        ...
        var defaultPerms = DefaultPermissions().ToDictionary(p => p.ModuleKey, StringComparer.OrdinalIgnoreCase);

        foreach (var row in rows)
        {
            if (defaultPerms.TryGetValue(row.ModuleKey, out var dto))
            {
                row.CanView = dto.CanView;
                row.CanAdd = dto.CanAdd;
                row.CanEdit = dto.CanEdit;
    ...
    private static List<RolePermissionDto> DefaultPermissions()
    {
        return SystemModules.All
            .Select(m => new RolePermissionDto(m.Key, m.DisplayName, CanView: true, CanAdd: false, CanEdit: false, CanDelete: false))
            .ToList();
    }
}
